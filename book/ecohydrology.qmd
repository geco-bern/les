...
$$ {#eq-partialpressures}
<!-- You'd need to use a matrix to display multiple equations in multiple lines. -->
...
<!-- You need to write valid latex expressions otherwise they are not properly displayed. -->
...
#| code-fold: true
#| label: fig-esat
#| fig-cap: "Saturation vapor pressure as a function of air temperature."
...
### Rooting-zone water-storage capacity {#sec-rzwsc}
...
knitr::include_graphics("images/cwd_sites_ecohydrology_SAVE.png")
```
...
Furthermore, water-limited conditions may be temporally limited and interspersed by energy-limited periods. Also, the water bucket model, as formulated above, suggests that water-limitation sets in at the point when soil moisture falls below the field capacity. However, as long as the net radiation and the atmospheric vapor pressure deficit are not "excessive", the demand for transpiration (@eq-transpiration-demand) may be met by the supply (@eq-hydraulicsupply) without substantial stomatal closure during the daytime.
...
The slope of the initial linear increase of fAPAR vs. *P*/PET reflects the water-carbon coupling. The amount of active green and transpiring foliage area is limited by water availability. research has shown that the relationship shown in @fig-donohue is shifting such that the slope of the initial increase tends to steepen over time [@donohue13grl; @ukkola16natcc]. This is related to the fact that under rising CO~2~, stomatal conductance tends to increase (@fig-constant-chi) and the water use efficiency is enhanced (@sec-wue). In other words, a larger area of green leaves per unit ground area can be sustained for a given level of aridity.
<!-- According to the figure, g_s increases with CO2 concentration. -->
