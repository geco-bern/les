...
Through its influence on GPP, as described by @eq-luemodel, solar radiation the "engine" of CO~2~ uptake by the terrestrial ecosystems through photosynthesis. Temporal and spatial patterns of GPP are strongly driven by patterns in solar radiation. In this subsection, the processes are introduced which are responsible for these patterns.
...
However, the FvCB model, on its own, does not allow us to model the LUE term in @eq-luemodel. An important additional ingredient is the stomatal conductance ($g_s$) which is relevant for determining the ratio of ambient to leaf-internal CO~2~ concentrations. It is described in @sec-stomatal-regulation.
