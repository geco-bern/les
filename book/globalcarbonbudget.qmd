...
<!-- XXX revise: -->
<!-- - make absolutely clear the difference between fluxes and pools.  -->
<!-- - ... and that the sink flux decreases to zero once a steady-state is attained. -->
...
Energy from solar radiation is absorbed and consumed to convert atmospheric C, present in its (oxidized) gaseous form CO~2~, into C in carbohydrates and biomass. In other words, biomass stores solar energy. This energy can be released again, not only through combustion (fire), but also through consumption of organic matter by heterotrophic organisms--animals (including humans), fungi, and bacteria. Fire and heterotrophic consumption release CO~2~ and thus close the cycle between land and the atmosphere.
...
## The pre-industrial carbon cycle {#sec-preindustrialcarboncycle}
...
::: {.callout-note}
The concentration of CO~2~ in the atmosphere is measured in parts per million (ppm), expressed as a dry-air mole fraction. Therefore, ppm is an abbreviation for micromoles per mole dry air. Since the total mass of the atmosphere is relatively well-known and CO~2~ is relatively well-mixed in the troposphere within one year, the CO~2~ concentration can directly be converted into a total atmospheric mass of C in the form of CO~2~ [@ballantyne12nat]:
...
<!-- Ex 1. cannot be done unless one also gives the input flux. -->
1. Express the steady-state pool size $C^\ast$ as a function of the in-flux and the turnover time $\tau$.
2. Consider the C pool of the intermediate and deep sea (37,100 PgC) in @fig-carbon-pools-fluxes-ipcc (bottom right). What is the turnover time of C in that pool?
...
$$
B_\mathrm{IM} = E_\mathrm{FOS} + E_\mathrm{LUC} - (G_\mathrm{atm} + S_\mathrm{ocean} + S_\mathrm{land})
$$ {#eq-budget_imbalance}
 
Today, the global carbon budget is well specified thanks to reliable estimates of its individual components. The budget imbalance is only a fraction of the total emissions. Until recently, "bottom-up" estimates of $S_\mathrm{land}$ by land C cycle models were deemed insufficient for providing reliable estimates. Instead, $S_\mathrm{land}$ was estimated as the budget residual by neglecting $B_\mathrm{IM}$ and rearranging terms in @eq-budget_imbalance to solve for $S_\mathrm{land}$. In fact, when the IPCC First Assessment Report was published in 1990, the existence of C sink on land was not known, and it was treated as the budget imbalance and referred to as the 'missing sink'. It was stated that "there are possible processes on land which could account for the missing CO~2~ (but it has not been possible to verify them)".

It was only later that the existence of a terrestrial C sink could be more firmly established and quantified thanks to parallel measurements of the atmospheric O~2~ and CO~2~ concentrations [@keeling96nat]. (In fact, the ratio O~2~/N~2~ is measured to avoid the much larger measurement uncertainty in absolute O~2~ measurements). The ratios of O~2~:CO~2~ are known from the stoichiometric molecular formulae for photosynthesis (O~2~:CO~2~ = 1.1), fossil fuel combustion (O~2~:CO~2~ = 1.4) and ocean uptake (O~2~:CO~2~ = 0). An ocean O~2~ source from marine organisms has to be factored in. Given the total C emissions from fossil fuel combustion, the net land C balance can thus be calculated and the calculation visualized geometrically (@fig-o2_n2_co2). Note that net emissions from land use change are not considered and what is termed the "*S*~land~" in @fig-o2_n2_co2 is actually the net $(S_\mathrm{land} - E_\mathrm{LUC})$ in @eq-carbon_budget.

```{r echo=FALSE}
#| label: fig-o2_n2_co2
#| fig-cap: "Global land and ocean carbon sinks deduced from changes in atmospheric CO~2~ and O~2~ concentrations. The figure is an updated version of Fig. 2 from @keeling96nat, taken from a recent update of the atmospheric carbon and oxygen budget by @li22aas. In this figure, what is termed '*S*~land~' corresponds to (*S*~land~ - *E*~LUC~) as defined in @eq-carbon_budget."
#| out-width: 60%
knitr::include_graphics("images/o2_n2_co2.png")
```

In a mathematical sense, the constraint from atmospheric oxygen can be understood as a second equation, necessary for solving for two unknowns $S_\mathrm{land}$ and $S_\mathrm{ocean}$. Their sum must equal (well-known) emissions $E_\mathrm{FOS} + E_\mathrm{LUC}$ minus the atmospheric growth rate $G_\mathrm{atm}$. This yields only one equation--that for the C budget (@eq-carbon_budget). The oxygen budget provides a second equation and thus enables solving for two unknowns. In a similar fashion, the isotopic composition of C (the ratio of ^13^C:^12^C) also provides a second equation [@joosbruno98gbc]. The history of ^13^C can be measured on ice cores and $(S_\mathrm{land} - E_\mathrm{LUC})$ can thus be reconstructed for the past (@fig-land_c_balance_past). Note that in order to separate $S_\mathrm{land}$ from the budget, independent estimates of $E_\mathrm{LUC}$ are required. 
...
# Data from Friedlingstein et al. 2024. Downloaded excel file 'Global 
# Carbon Budget v2025 from
...
df <- read_csv(here("data/Global_Carbon_Budget_2024v1_tabHistoricalBudget.csv")) |> 
  rename(
    year = Year,
...
df <- df |> 
  ...
  # consider only years after 1850
  filter(year >= 1850) |> 
  
  # Assume zero cement carbonation sink for years where no data is available 
  # (pre-1930)
  mutate(s_cement = ifelse(is.na(s_cement), 0, s_cement)) |> 
...
df |> 
  ...
  scale_color_manual(
    ...
    labels = "Difference between land \nsink, defined as budget residual, \nminus simulated land sink"
    ) +
  theme_minimal() +
  theme(
...
ggsave(here::here("book/images/globalcarbonbudget.pdf"), width = 6, height = 6)
```

## Understanding the land C sink {#sec-sink}
...
<!-- xxx to add: explicitly mention ENSO, Cox et al study. -->
...
<!-- XXX add exercise question: how is each global C budget component estimated? -->
